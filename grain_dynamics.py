...
class Grain:
    """
    Core unit in the Collapse Geometry framework.
    
    A Grain embodies the duality between structure and potential, capable
    of existing in superposition (unbounded potential) or defined states.
    Structure emerges through collapse events, which increase saturation.
    
    Properties emerge from the relational dynamics rather than being predefined
    in absolute space. Position, momentum, and properties arise purely from
    relational constraints.
    """
    
    def __init__(self, grain_id: str):
        """
        Initialize a grain with default values.
        ...
        # Core properties in polarity space
        self.awareness = 0.0  # Awareness level [-1.0 to 1.0]
        self.polarity = 0.0   # Polarity bias [-1.0 to 1.0]
        self.coherence = 1.0  # Phase coherence [0.0 to 1.0]
        
        # Quantum properties
        self.unbounded_potential = True  # In superposition state initially
        self.degrees_of_freedom = 1.0    # Maximum freedom initially
        self.collapse_metric = 0.0       # No collapse history initially
        
        # Emergent properties from structure
        self.grain_saturation = 0.0  # No saturation initially
        self.grain_activation = 0.0  # No activation initially
        ...
        self.opposite_id = None   # ID of opposite grain if it exists
        
        # Tensor properties
        self.coherence_tensor = None     # 3x3 coherence tensor
        self.coherence_eigenvalues = None # Eigenvalues of coherence tensor
        self.coherence_eigenvectors = None # Eigenvectors of coherence tensor
        self.flow_vector = np.zeros(3)   # Flow vector in phase space
        self.gradient_tensor = None      # Gradient tensor for flow dynamics
        self.flow_divergence = 0.0       # Divergence of flow field
        
        # Ancestry structure
        self.ancestry_depth = 0    # Depth of ancestry tree
        self.ancestry_breadth = 0  # Breadth of ancestry tree
        self.parent_id = None      # Direct parent if applicable
        self.ancestry_polarity = {} # Maps ancestor_id -> polarity_influence
        self.ancestry_curvature = 0.0  # Curvature from ancestry structure
        
        # Curvature properties
        self.local_curvature = 0.0    # Local curvature value
        self.curvature_gradient = np.zeros(3)  # Curvature gradient vector
        
        # Toroidal position - emerges from relational constraints
        self.theta = random.random() * 2 * math.pi  # [0, 2π]
        self.phi = random.random() * 2 * math.pi    # [0, 2π]
        
        # Neighborhood tracking
        self.neighborhood = set()  # Set of neighbor grain IDs

    def is_in_superposition(self) -> bool:
        """
        Check if the grain is in a superposition state.
        
        Returns:
            True if in superposition, False otherwise
        """
        # Use the compatible function from grain_dynamics
        return is_in_superposition(
            self.polarity,
            self.awareness,
            self.ancestry,
            self.collapse_metric,
            self.relations,
            self.coherence_eigenvalues,
            self.unbounded_potential
        )
    
    def update_saturation(self):
        """Update grain saturation based on current properties"""
        self.grain_saturation = calculate_grain_saturation(
            self.polarity,
            self.awareness,
            self.ancestry,
            self.collapse_metric,
            ...
            self.coherence,
            self.ancestry_depth,
            self.ancestry_breadth,
            self.coherence_eigenvalues,
            self.flow_divergence,
            self.local_curvature,
            self.curvature_gradient
        )
        
        # Also update degrees of freedom
        self.update_degrees_of_freedom()
    
    def update_degrees_of_freedom(self):
        """Update degrees of freedom based on current properties"""
        self.degrees_of_freedom = calculate_degrees_of_freedom(
            self.grain_saturation,
            self.ancestry_depth,
            self.ancestry_breadth,
            len(self.ancestry),
            self.relations,
            ...
            self.coherence,
            self.local_curvature,
            self.is_in_superposition()
        )
    
    def update_coherence_tensor(self):
        """Update coherence tensor based on flow and curvature"""
        tensor_data = update_coherence_tensor(
            self.coherence,
            self.flow_vector,
            self.curvature_gradient,
            self.local_curvature,
        ...
        self.coherence_tensor = tensor_data['tensor']
        self.coherence_eigenvalues = tensor_data['eigenvalues']
        self.coherence_eigenvectors = tensor_data['eigenvectors']
    
    def update_flow_vector(self, relational_vectors: Dict[str, np.ndarray], gradients: Dict[str, float]):
        """
        Update flow vector based on relational vectors and gradients
        
        Args:
            relational_vectors: Dict mapping related_id -> relational vector
            gradients: Dict mapping related_id -> gradient value
        """
        flow_data = update_flow_vector(
            self.flow_vector,
            relational_vectors,
            gradients,
            self.curvature_gradient,
        ...
        self.flow_vector = flow_data['flow_vector']
        self.gradient_tensor = flow_data['gradient_tensor']
        self.flow_divergence = flow_data['flow_divergence']
    
    def update_relation(self, related_id: str, relation_strength: float):
        """
        Update relation to another grain
        
        Args:
            related_id: ID of related grain
            relation_strength: Strength of relation [-1.0 to 1.0]
        """
        # Update or add relation
        self.relations[related_id] = relation_strength
    
    def update_relation_memory(self, related_id: str, memory_strength: float):
        """
        Update relational memory for another grain
        
        Args:
            related_id: ID of related grain
            memory_strength: Strength of memory [-1.0 to 1.0]
        """
        # Initialize memory dict if needed
        if not hasattr(self, 'relation_memory') or self.relation_memory is None:
            self.relation_memory = {}
            
        # Update or add memory
        self.relation_memory[related_id] = memory_strength
    
    def set_opposite_state(self, opposite_id: str):
        """
        Set this grain as the opposite of another grain
        
        Args:
            opposite_id: ID of the opposite grain
        """
        self.opposite_id = opposite_id
        
        # When explicitly set as opposite, polarity often becomes negative
        # of the other grain's polarity (handled by the manifold)
    
    def update_neighborhood(self, all_grain_ids: Set[str], threshold: float = 0.5):
        """
        Update neighborhood based on relation strengths
        
        Args:
            all_grain_ids: Set of all grain IDs in system
            threshold: Relation strength threshold
        """
        self.neighborhood = update_relational_neighborhood(
            self.relations,
            all_grain_ids,
            self.id,
            threshold,
    ...
    def process_collapse(self, collapse_strength: float, is_structure: bool = True):
        """
        Process a collapse event affecting this grain
        
        Args:
            collapse_strength: Strength of collapse [0.0 to 1.0]
            is_structure: True for structure collapse, False for decay
        ...
        if is_structure:
            # Positive collapse (structure-forming)
            result = process_structure_collapse(
                self.awareness,
                self.polarity,
                self.coherence,
                collapse_strength,
                ...
                self.local_curvature,
                self.curvature_gradient,
                self.ancestry_curvature
            )
        else:
            # Negative collapse (structure-decaying)
            result = process_decay_collapse(
                self.awareness,
                self.polarity,
                self.coherence,
                -collapse_strength,  # Negate for decay process
                self.grain_saturation,
                self.is_in_superposition(),
                self.local_curvature,
                self.curvature_gradient,
        ...
        # Update properties
        self.awareness = result['awareness']
        self.polarity = result['polarity']
        self.coherence = result['coherence']
        
        # Update other derived properties
        self.update_saturation()
        self.update_coherence_tensor()
        
        return result
    
    def resolve_from_superposition(self, awareness_level: float = None) -> bool:
        """
        Resolve grain from superposition to a defined state
        
        Args:
            awareness_level: Optional target awareness level
            
        Returns:
            True if successfully resolved, False otherwise
        """
        # Skip if not in superposition
        if not self.is_in_superposition():
            return False
            
        # Get resolution values with curvature effects
        result = resolve_from_superposition(
            awareness_level,
            self.polarity,
            self.local_curvature,
            self.curvature_gradient
    ...
    def predict_cascade_direction(self) -> Dict[str, Any]:
        """
        Predict probability and direction of cascade collapse
        
        Returns:
            Dictionary with cascade prediction metrics
        """
        relational_vectors = {}  # This would need to be populated by calling code
        
        return predict_cascade_direction(
            self.polarity,
            self.grain_saturation,
            self.coherence,
            self.is_in_superposition(),
    ...
    def update_curvature(self, collapse_strength: float, source_id: str = None, 
                         relational_vectors: Dict[str, np.ndarray] = None):
        """
        Update curvature based on collapse event
        
        Args:
            collapse_strength: Signed collapse strength
            source_id: Optional source grain ID
            relational_vectors: Optional dict of relation vectors
        """
        curvature_data = calculate_curvature_effects(
            collapse_strength,
            source_id,
            relational_vectors,
            self.flow_vector,
        ...
        # Update curvature properties
        self.local_curvature = curvature_data['local_curvature']
        self.curvature_gradient = curvature_data['curvature_gradient']
        self.ancestry_curvature = curvature_data['ancestry_curvature']
    
    def update_toroidal_position(self, theta: float, phi: float):
        """
        Update toroidal position coordinates
        
        Args:
            theta: Theta coordinate [0, 2π]
            phi: Phi coordinate [0, 2π]
        """
        self.theta = theta
        self.phi = phi
...
class RelationalGrainSystem:
    """
    System for managing collections of related grains.
    Provides aggregate operations and optimized access patterns.
    """
    
    def __init__(self):
        """Initialize an empty grain system"""
        self.grains = {}  # Maps grain_id -> Grain
        self.time = 0.0
        self.neighborhoods = {}  # Maps grain_id -> set of neighbor ids
    ...
    def connect_grains(self, grain_id1: str, grain_id2: str, relation_strength: float = 0.5):
        """
        Connect two grains with a relation
        ...
        # Update relations
        self.grains[grain_id1].update_relation(grain_id2, relation_strength)
        self.grains[grain_id2].update_relation(grain_id1, relation_strength)
        
        # Update neighborhoods
        if grain_id1 not in self.neighborhoods:
            self.neighborhoods[grain_id1] = set()
        if grain_id2 not in self.neighborhoods:
            self.neighborhoods[grain_id2] = set()
            
        self.neighborhoods[grain_id1].add(grain_id2)
        self.neighborhoods[grain_id2].add(grain_id1)
    
    def update_neighborhoods(self, threshold: float = 0.5):
        """
        Update all grain neighborhoods based on relation strengths
        
        Args:
            threshold: Relation strength threshold for neighborhood inclusion
        """
        # Get all grain IDs
        all_ids = set(self.grains.keys())
        
        # Update each grain's neighborhood
        for grain_id, grain in self.grains.items():
            neighborhood = update_relational_neighborhood(
                grain.relations,
                all_ids,
                grain_id,
                threshold,
                grain.is_in_superposition(),
                grain.local_curvature
            )
            
            # Store in system neighborhoods
            self.neighborhoods[grain_id] = neighborhood
            
            # Update grain's own neighborhood
            grain.neighborhood = neighborhood
    
    def find_collapse_cascade_pathways(self) -> Dict[str, List[Dict[str, Any]]]:
        """
        Find potential collapse cascade pathways in the system.
        These represent lightlike propagation paths for structure and decay.
        
        Returns:
            Dictionary with 'structure' and 'decay' pathway lists
        """
        # Pathways for structure and decay
        pathways = {
            'structure': [],
            'decay': []
        }
        
        # Refresh neighborhoods first
        self.update_neighborhoods()
        
        # Find seeds for cascade pathways
        lightlike_seeds = []
        high_potential_seeds = []
        
        for grain_id, grain in self.grains.items():
            # Skip grains with no neighbors
            if grain_id not in self.neighborhoods or not self.neighborhoods[grain_id]:
                continue
                
            # Get cascade prediction
            prediction = grain.predict_cascade_direction()
            
            # Check for lightlike properties (low saturation = high propagation)
            if prediction['lightlike_factor'] > 0.7:
                lightlike_seeds.append((grain_id, prediction))
            
            # Check for high cascade potential
            if prediction['cascade_potential'] > 0.7:
                high_potential_seeds.append((grain_id, prediction))
        
        # Start with lightlike seeds, then high potential seeds
        all_seeds = lightlike_seeds + high_potential_seeds
        
        # Track visited grains to avoid cycles
        visited = set()
        
        # Process seeds
        for seed_id, prediction in all_seeds:
            if seed_id in visited:
                continue
                
            # Determine pathway type
            structure_prob = prediction['structure_prob']
            decay_prob = prediction['decay_prob']
            
            pathway_type = 'structure' if structure_prob > decay_prob else 'decay'
            
            # Find pathway
            pathway = self._trace_lightlike_pathway(seed_id, pathway_type, visited)
            
            if pathway and len(pathway['grains']) >= 3:
                # Valid pathway found
                pathways[pathway_type].append(pathway)
        
        return pathways
    
    def _trace_lightlike_pathway(self, start_id: str, pathway_type: str, visited: Set[str]) -> Dict[str, Any]:
        """
        Trace a lightlike pathway from a starting grain
        
        Args:
            start_id: Starting grain ID
            pathway_type: Type of pathway ('structure' or 'decay')
            visited: Set of visited grain IDs
            
        Returns:
            Pathway dictionary or None if no valid pathway
        """
        # Initialize pathway
        pathway = {
            'type': pathway_type,
        ...
        # Initial stats
        lightlike_count = 1 if current_grain.grain_saturation < 0.2 else 0
        polarity_sum = current_grain.polarity
        coherence_sum = current_grain.coherence
        ...
        # Trace pathway
        while len(pathway['grains']) < max_length:
            # Get neighborhood
            if current_id not in self.neighborhoods:
                break
                
            neighbors = self.neighborhoods[current_id]
            
            # Find next grain in pathway
            next_id = None
            best_score = -1.0
            ...
            for neighbor_id in neighbors:
                ...
                # Get neighbor grain
                neighbor = self.grains.get(neighbor_id)
                if not neighbor:
                    continue
                
                # Calculate propagation score
                score = self._calculate_propagation_score(
                    current_grain, neighbor, pathway_type
                )
            ...
            # Check if found next grain
            if next_id is None or best_score < 0.4:
                break
                
            # Add to pathway
            pathway['grains'].append(next_id)
            visited.add(next_id)
            ...
            # Update stats
            if current_grain.grain_saturation < 0.2:
                lightlike_count += 1
                
            polarity_sum += current_grain.polarity
            coherence_sum += current_grain.coherence
            
            # Path successful if we've added at least 2 grains
            if len(pathway['grains']) >= 3:
                found_path = True
        ...
        # Calculate final pathway stats
        pathway_length = len(pathway['grains'])
        pathway['length'] = pathway_length
        pathway['lightlike_ratio'] = lightlike_count / pathway_length
        pathway['avg_polarity'] = polarity_sum / pathway_length
        pathway['avg_coherence'] = coherence_sum / pathway_length
        
        # Calculate pathway potential
        polarity_alignment = 1.0 if pathway_type == 'structure' else -1.0
        polarity_factor = 0.5 + min(0.5, abs(pathway['avg_polarity']) * 0.5)
        if pathway['avg_polarity'] * polarity_alignment < 0:
            polarity_factor *= 0.5  # Penalize wrong polarity direction
            
        pathway['potential'] = pathway['lightlike_ratio'] * 0.6 + polarity_factor * 0.4
        
        return pathway
    
    def _calculate_propagation_score(self, source: Grain, target: Grain, pathway_type: str) -> float:
        """
        Calculate propagation score between source and target grains
        ...
        Returns:
            Propagation score [0.0 to 1.0]
        """
        # Calculate base propagation
        collapse_strength = 0.5 if pathway_type == 'structure' else -0.5
        
        # Calculate propagation using collapse function
        propagation = calculate_collapse_propagation(
            source.polarity,
            source.grain_saturation,
            source.coherence,
            target.polarity,
            collapse_strength,
            source.coherence_eigenvalues,
            source.coherence_eigenvectors,
            source.flow_vector,
            None,  # relational_vector
            source.local_curvature,
            target.local_curvature
        )
        
        # Normalize score
        if pathway_type == 'structure':
            # For structure, higher positive value = better score
            score = max(0.0, min(1.0, propagation))
        else:
            # For decay, more negative value = better score
            score = max(0.0, min(1.0, abs(propagation)))
        
        # Bonus for lightlike targets
        if target.grain_saturation < 0.2:
            score *= 1.2
            
        # Bonus for freedom
        freedom_bonus = target.degrees_of_freedom * 0.2
        score += freedom_bonus
        
        # Cap at 1.0
        return min(1.0, score)

def create_random_grain(grain_id: str) -> Grain:
    """
    Create a random grain with initial state values.
    Grain starts in superposition by default.
    ...
    # Random toroidal position
    grain.theta = random.random() * 2 * math.pi
    grain.phi = random.random() * 2 * math.pi
